...
import { FOLDERS_CSS } from './folders-styles.js';
...
export function createFoldersModule({ storage, Core, Logger, getPanelUI, getCounterModule, applyTheme, getTheme, injectStyles: injectCSS }) {
    ...
    return {
                ...
                for (const folderId of this.data.folderOrder) {
                    ...
                    const matched = folder.rules.some(rule => {
                        ...
                        if (rule.type === 'regex' && rule.value) {
                            try {
                                if (rule.value.length > 100) return false;
        ...
        markSidebarChats() {
            ...
            chats.forEach(chat => {
                ...
                if (folderId && this.data.folders[folderId]) {
                    ...
                    const safeColor = /^#[0-9a-fA-F]{3,8}$/.test(folder.color) ? folder.color : '#8ab4f8';
                    dot.style.cssText = `display:inline-block;width:6px;height:6px;border-radius:50%;background:${safeColor};margin-right:6px;flex-shrink:0;vertical-align:middle;`;
        ...
        createFolderRow(folderId, folder, chats) {
            ...
            editBtn.setAttribute('aria-label', `Edit ${folder.name}`);
            editBtn.onclick = (e) => {
                e.stopPropagation();
        ...
        showFolderModal(folderId, title, currentName, currentColor) {
            ...
            overlay.onkeydown = (e) => {
                ...
                if (e.key === 'Tab') {
                    const focusable = overlay.querySelectorAll('input,button,select,[tabindex]:not([tabindex="-1"])');
